...
- В `hub-shell` подключен `ProfilesListWidget` из локального пакета AprilProfile (`../april-profile-1/frontend/packages/profile-ui/dist`) как временная интеграция до публикации в registry.
...
- До публикации пакета в registry используется прямое локальное подключение из `april-profile-1`.
