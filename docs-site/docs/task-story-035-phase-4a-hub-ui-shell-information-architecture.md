...
Минимальный контракт по-прежнему описан в [`docs/WIDGET_CONTRACTS.md`](../../docs/WIDGET_CONTRACTS.md). В shell добавлены провайдер **`HubHostContextProvider`** (навигация + срез маршрута поверх tenant/auth/telemetry) и **`ShellToastProvider`** для унифицированных уведомлений об успехе/ошибке. Подключение нового виджета: см. **§8** в том же документе.
