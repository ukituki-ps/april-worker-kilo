...
- [x] Создана рабочая ветка `feature/aprilhub-implementation` для реализации AprilHub (устарела — все эпики 001–010 merged в `develop` отдельными PR'ами)
- [x] До завершения эпиков `001-010` изменения по AprilHub вести в `feature/aprilhub-implementation` → **закрыто, все эпики merged в `develop` по частям**
- [x] После закрытия эпиков `001-010` — подготовить финальный PR `feature/aprilhub-implementation` -> `develop` → **закрыто, финальный PR не потребовался: каждый эпик шёл отдельным PR**
...
- [x] `027`: Исполнение внешней задачи 028 из `april-profile-1` (анализ docs + реализация + двойной отчёт)
...
- [x] `031`: Исполнение внешней задачи 035 из `april-profile-1` (shell/IA/HostContext по 4a.0 + docs-site во внешнем репо + двойной отчёт) — **дубликат 025, закрыто через PR #46**
...
- [x] `033`: Наблюдаемость ошибок фронтенда/API: архитектурное решение и документационный контур (AprilHub + AprilProfile)
...
- [x] `037`: Реализация Sentry и error telemetry в AprilHub (`hub-shell`/`hub-bff`)
- [x] `038`: Реализация Sentry и error telemetry в AprilProfile (`april-profile-1`)
- [x] `039`: Аналог задачи 033 для AprilProfile: архитектура и документация error telemetry (Sentry + Loki/Prometheus)
- [x] `040`: Единый Grafana-дэшборд ошибок frontend/backend для dev-стенда (панели, фильтры, drill-down, runbook)
...
- [x] `042`: Интеграция внешнего `profiles-widget` в сайдбар AprilHub (замена legacy списка профилей, host-обвязка, e2e smoke)
- [x] `043`: Минималистичный сайдбар AprilHub — оставить только `Профиль — список`, убрать остальные разделы/маршруты/запросы
- [x] `044`: Auth-only контур AprilHub — оставить авторизацию и пустой сайдбар, убрать весь продуктовый runtime
- [x] `045`: Вернуть в AprilHub раздел `Профили` в сайдбаре и встроить `profiles-widget` как production-ready модуль (host-context/auth/tenant/error/telemetry/release-gate)
...
- [x] `051`: Сайдбар «Шаблоны» + интеграция `entity-types-widget` из AprilProfile в `hub-shell` (по образцу «Профили») — [`tasks/051-aprilhub-sidebar-entity-types-widget/TASK.md`](./tasks/051-aprilhub-sidebar-entity-types-widget/TASK.md)
...
- [x] `027-aprilhub-execute-external-task-028-april-profile-1`: изучить задачу 028 и документацию в `april-profile-1`, реализовать требования и оформить отчёт в обоих репозиториях
...
- [x] `031-aprilhub-execute-external-task-035-april-profile-1`: **дубликат 025** — shell/IA/HostContext уже закрыт через PR #46 в задаче 025; отдельная папка задач не создавалась
...
- [x] `033-observability-error-telemetry-architecture-docs`: зафиксировать архитектурное решение по сбору/корреляции ошибок (Sentry + Loki/Prometheus) и обновить docs/архитектурные артефакты
...
- [x] `037-aprilhub-sentry-implementation`: реализовать frontend/runtime error capture в `hub-shell` и интеграцию с текущим observability-контуром AprilHub
- [x] `038-aprilprofile-sentry-implementation`: реализовать error capture и операционную интеграцию в `april-profile-1` (внешняя постановка + двойной отчёт)
- [x] `039-aprilprofile-observability-error-telemetry-architecture-docs`: отдельная внешняя задача (аналог `033`) на архитектурное решение и документационный контур error telemetry в `april-profile-1` с двойным отчётом
- [x] `040-aprilhub-unified-error-dashboard`: собрать полноценный Grafana-дэшборд ошибок всех уровней (`frontend` + `hub-bff` + downstream) для dev-стенда и задокументировать triage-поток
...
- [x] `042-aprilhub-profiles-widget-sidebar-integration`: перевести пункт сайдбара `Профиль — список` на внешний `profiles-widget` из `april-profile-1`, удалить legacy host-list реализацию и закрепить e2e smoke
- [x] `043-aprilhub-sidebar-minimal-profile-list-only`: упростить sidebar/routing AprilHub до единственного раздела `Профиль — список`, удалить runtime legacy-маршруты и лишние host-виджеты
...
- [x] `045-aprilhub-sidebar-profiles-widget-section`: вернуть в sidebar раздел `Профили` и встроить `profiles-widget` с production-ready host-обвязкой (auth/tenant/correlation, callbacks, e2e smoke, release-gate)
...
| `026` | Исполнение внешней задачи 026 из `april-profile-1` (анализ docs + реализация + двойной отчёт) | ✅ Выполнено | [`tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/TASK.md`](./tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/TASK.md), [`tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/PLAN.md`](./tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/PLAN.md), [`tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/REPORT.md`](./tasks/026-aprilhub-align-docs-with-april-profile-design-system-approach/REPORT.md) |
| `027` | Исполнение внешней задачи 028 из `april-profile-1` (анализ docs + реализация + двойной отчёт) | ✅ Выполнено | [`tasks/027-aprilhub-execute-external-task-028-april-profile-1/TASK.md`](./tasks/027-aprilhub-execute-external-task-028-april-profile-1/TASK.md), [`REPORT.md`](./tasks/027-aprilhub-execute-external-task-028-april-profile-1/REPORT.md) |
...
| `031` | **Дубликат 025** — shell/IA/HostContext закрыт через PR [#46](https://github.com/ukituki-ps/april-worker/pull/46) | ✅ Выполнено | [`../025-aprilhub-execute-external-task-035-april-profile-1/REPORT.md`](./tasks/025-aprilhub-execute-external-task-035-april-profile-1/REPORT.md) |
...
| `037` | Реализация Sentry и error telemetry в AprilHub (`hub-shell`/`hub-bff`) | ✅ Выполнено | [`tasks/037-aprilhub-sentry-implementation/TASK.md`](./tasks/037-aprilhub-sentry-implementation/TASK.md), [`tasks/037-aprilhub-sentry-implementation/PLAN.md`](./tasks/037-aprilhub-sentry-implementation/PLAN.md), [`tasks/037-aprilhub-sentry-implementation/REPORT.md`](./tasks/037-aprilhub-sentry-implementation/REPORT.md) |
| `038` | Реализация Sentry и error telemetry в AprilProfile (`april-profile-1`) | ✅ Выполнено | [`tasks/038-aprilprofile-sentry-implementation/TASK.md`](./tasks/038-aprilprofile-sentry-implementation/TASK.md), [`tasks/038-aprilprofile-sentry-implementation/REPORT.md`](./tasks/038-aprilprofile-sentry-implementation/REPORT.md) |
| `039` | Аналог задачи 033 для AprilProfile: архитектура и документация error telemetry (Sentry + Loki/Prometheus) | ✅ Выполнено | [`tasks/039-aprilprofile-observability-error-telemetry-architecture-docs/TASK.md`](./tasks/039-aprilprofile-observability-error-telemetry-architecture-docs/TASK.md), [`tasks/039-aprilprofile-observability-error-telemetry-architecture-docs/REPORT.md`](./tasks/039-aprilprofile-observability-error-telemetry-architecture-docs/REPORT.md) |
| `040` | Единый Grafana-дэшборд ошибок frontend/backend для dev-стенда | ✅ Выполнено | [`tasks/040-aprilhub-unified-error-dashboard/TASK.md`](./tasks/040-aprilhub-unified-error-dashboard/TASK.md), [`tasks/040-aprilhub-unified-error-dashboard/PLAN.md`](./tasks/040-aprilhub-unified-error-dashboard/PLAN.md), [`tasks/040-aprilhub-unified-error-dashboard/REPORT.md`](./tasks/040-aprilhub-unified-error-dashboard/REPORT.md) |
...
| `042` | Интеграция внешнего `profiles-widget` в сайдбар AprilHub (замена legacy host-list, host-обвязка, smoke) | ✅ Выполнено | [`tasks/042-aprilhub-profiles-widget-sidebar-integration/TASK.md`](./tasks/042-aprilhub-profiles-widget-sidebar-integration/TASK.md), [`tasks/042-aprilhub-profiles-widget-sidebar-integration/PLAN.md`](./tasks/042-aprilhub-profiles-widget-sidebar-integration/PLAN.md), [`tasks/042-aprilhub-profiles-widget-sidebar-integration/REPORT.md`](./tasks/042-aprilhub-profiles-widget-sidebar-integration/REPORT.md) |
| `043` | Минималистичный сайдбар AprilHub: только `Профиль — список` (удаление лишних разделов/маршрутов/host-runtime) | ✅ Выполнено | [`tasks/043-aprilhub-sidebar-minimal-profile-list-only/TASK.md`](./tasks/043-aprilhub-sidebar-minimal-profile-list-only/TASK.md), [`tasks/043-aprilhub-sidebar-minimal-profile-list-only/PLAN.md`](./tasks/043-aprilhub-sidebar-minimal-profile-list-only/PLAN.md), [`tasks/043-aprilhub-sidebar-minimal-profile-list-only/REPORT.md`](./tasks/043-aprilhub-sidebar-minimal-profile-list-only/REPORT.md) |
...
| `045` | Раздел `Профили` в sidebar AprilHub + интеграция `profiles-widget` (production-ready host wiring и release-gate) | ✅ Выполнено | [`tasks/045-aprilhub-sidebar-profiles-widget-section/TASK.md`](./tasks/045-aprilhub-sidebar-profiles-widget-section/TASK.md), [`tasks/045-aprilhub-sidebar-profiles-widget-section/PLAN.md`](./tasks/045-aprilhub-sidebar-profiles-widget-section/PLAN.md), [`tasks/045-aprilhub-sidebar-profiles-widget-section/REPORT.md`](./tasks/045-aprilhub-sidebar-profiles-widget-section/REPORT.md) |
