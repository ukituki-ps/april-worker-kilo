...
Обеспечить полное визуальное покрытие неадминского пользовательского контура Keycloak так, чтобы для обычного пользователя не отображались страницы в базовом стиле Keycloak.
...
- Определить целевой объём кастомизации Keycloak theme для dev/runtime контура:
  - login page,
  - error/info states,
  - required actions и другие неадминские auth-экраны пользовательского сценария (включая reset/update/verify и эквивалентные flow-экраны, если они активированы),
  - кнопки/поля/типографика/цвета в рамках дизайн-системы AprilHub.
- Обеспечить единый branded UX для desktop и mobile viewport без отдельных "базовых" fallback-страниц для обычного пользователя.
...
- Для неадминского контура не допускается отображение страниц в дефолтном визуале Keycloak; если экран включён в пользовательский flow, он должен быть брендирован под AprilHub.
- Кастомизация должна учитывать адаптивность (mobile-first friendly) и не деградировать UX на узких экранах.

## Критерии готовности (acceptance)
- [ ] Все неадминские пользовательские экраны Keycloak, доступные в активированных flow, визуально согласованы с дизайн-системой AprilHub.
- [ ] Обычный пользователь не видит страниц Keycloak в дефолтном (базовом) визуальном стиле на desktop и mobile viewport.
...
- [ ] В `REPORT.md` зафиксированы: карта покрытых экранов (desktop/mobile), состав theme-кастомизаций, проверки и ограничения/follow-up.
...
## Результат в отчёте
После выполнения оформить `REPORT.md`: какие неадминские экраны Keycloak кастомизированы (с фиксацией покрытия desktop/mobile), как подключена тема, какие e2e/smoke проверки выполнены и какие ограничения/доработки остаются.
